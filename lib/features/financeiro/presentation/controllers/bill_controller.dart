...
class BillController extends Notifier<AppState<List<Bill>>> {
  ...
  Future<void> checkoutBill(
    ...
    try {
      ...
      // 2. Prepare transactions list for batch insert
      final List<TransacaoFinanceira> txList = [];
      for (final payment in payments) {
        txList.add(
          TransacaoFinanceira(
            ...
            type: 'income',
            description:
                'Checkout Comanda - ${bill.clientName} (${payment.method.toUpperCase()})',
      ...
      // Add all transactions in one batch (which calls loadSummary once)
      await ref
          .read(transacoesControllerProvider.notifier)
          .addTransacoesBatch(txList);
...
final billControllerProvider =
    NotifierProvider<BillController, AppState<List<Bill>>>(
      () => BillController(),
    );
