...
import 'package:barber_osbao/features/financeiro/presentation/controllers/caixa_controller.dart';

class CaixaPage extends ConsumerStatefulWidget {
  const CaixaPage({super.key});

  @override
  ConsumerState<CaixaPage> createState() => _CaixaPageState();
}

class _CaixaPageState extends ConsumerState<CaixaPage> {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(caixaControllerProvider);
    ...
    final activeCaixa = state.data;
    if (activeCaixa == null) {
      return _buildClosedCaixaView(context);
    }
    return _buildOpenCaixaView(context, activeCaixa, isDark);
  }

  Widget _buildClosedCaixaView(BuildContext context) {
          ...
          children: [
            ...
            AppButton(
              ...
              onPressed: () {
                ...
                ref.read(caixaControllerProvider.notifier).abrir(val);
              },
            ),
          ],
        ),
  ...
  Widget _buildOpenCaixaView(BuildContext context, CaixaTurno caixa, bool isDark) {
    final movementsState = ref.watch(movimentosCaixaControllerProvider);
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            double entries = 0.0;
            double exits = 0.0;
            if (movementsState is AppSuccess<List<MovimentoCaixa>>) {
            ...
            final currentBalance = caixa.initialBalance + entries - exits;
            ...
            return GridView(
              ...
              children: [
                AppStatCard(
                  title: 'SALDO INICIAL',
                  value: 'R\$ ${caixa.initialBalance.toStringAsFixed(2)}',
        ...
        Wrap(
          ...
          children: [
            AppButton(
              ...
              onPressed: () => _showMovementDialog(context, 'sangria'),
            ),
            AppButton(
              label: 'Registrar Suprimento',
              icon: const Icon(Icons.add, size: 16),
              onPressed: () => _showMovementDialog(context, 'suprimento'),
            ),
            AppButton(
              label: 'Fechar Turno de Caixa',
              icon: const Icon(Icons.lock, size: 16),
              variant: AppButtonVariant.primary,
              onPressed: () => _showCloseCaixaDialog(context, caixa, movementsState),
            ),
          ],
        ),
  ...
  Widget _buildMovementsTable(AppState<List<MovimentoCaixa>> state, bool isDark) {
    if (state is AppLoading) {
      return const Center(child: CircularProgressIndicator(color: ThemeColors.primary));
    }
    if (state is AppError) {
    ...
    return AppTable(
      ...
      rows: data.map((mv) {
        final isInput = mv.type == 'entrada' || mv.type == 'suprimento';
        return AppTableRow(
          cells: [
            Text(mv.time),
            AppStatusChip(
              label: mv.type.toUpperCase(),
  ...
  void _showMovementDialog(BuildContext context, String type) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(type == 'sangria' ? 'Registrar Sangria (Retirada)' : 'Registrar Suprimento (Troco)', style: const TextStyle(color: Colors.white, fontSize: 16)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            AppInput(
        ...
        actions: [
          ...
          AppButton(
            ...
            onPressed: () {
              ...
              if (val > 0) {
                ref.read(caixaControllerProvider.notifier).registrarMovimentacao(
                      type,
                      val,
  ...
  void _showCloseCaixaDialog(BuildContext context, CaixaTurno caixa, AppState<List<MovimentoCaixa>> movementsState) {
    ...
    final totalExpected = caixa.initialBalance + entries - exits;
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Text('R\$ ${caixa.initialBalance.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontSize: 12)),
                      ],
                    ),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Total de Entradas em Dinheiro', style: TextStyle(color: Colors.grey, fontSize: 12)),
                        Text('R\$ ${entries.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontSize: 12)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Total de Saídas/Sangrias', style: TextStyle(color: Colors.grey, fontSize: 12)),
                        Text('R\$ ${exits.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontSize: 12)),
                      ],
                    ),
                    const Divider(),
                    Row(
        ...
        actions: [
          ...
          AppButton(
            ...
            onPressed: () {
              ...
              ref.read(caixaControllerProvider.notifier).fechar(totalExpected, reported);
              Navigator.pop(ctx);
            },
          ),
        ],
