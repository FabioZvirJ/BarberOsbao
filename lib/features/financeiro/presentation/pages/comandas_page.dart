...
import 'package:barber_osbao/features/financeiro/presentation/controllers/comanda_controller.dart';
...
class ComandasPage extends ConsumerStatefulWidget {
  const ComandasPage({super.key});

  @override
  ConsumerState<ComandasPage> createState() => _ComandasPageState();
}

class _ComandasPageState extends ConsumerState<ComandasPage> {
  final _comandaNameController = TextEditingController();

  @override
  void dispose() {
    _comandaNameController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(comandaControllerProvider);
        ...
        Row(
          ...
          children: [
            ...
            AppButton(
              ...
              onPressed: () => _showCreateComandaDialog(context),
            ),
          ],
        ),
  ...
  Widget _buildContent(AppState<List<Comanda>> state, bool isDark) {
    ...
    final openComandas = data.where((c) => c.status == 'open').toList();
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GridView.builder(
          ...
          itemCount: openComandas.length,
          itemBuilder: (context, index) {
            final comanda = openComandas[index];
            return _buildComandaCard(context, comanda);
          },
        );
      },
    );
  ...
  Widget _buildComandaCard(BuildContext context, Comanda comanda) {
    return AppCard(
      child: Column(
        ...
        children: [
          ...
          Text(
            '${comanda.itens.length} itens lançados',
          ...
          Row(
            ...
            children: [
              Text(
                'R\$ ${comanda.total.toStringAsFixed(2)}',
  ...
  void _showCreateComandaDialog(BuildContext context) {
    _comandaNameController.clear();
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: AppInput(
          ...
          controller: _comandaNameController,
        ...
        actions: [
          ...
          AppButton(
            label: 'Abrir',
            onPressed: () {
              if (_comandaNameController.text.isNotEmpty) {
                ...
                final newComanda = Comanda(
                  id: '',
                  clienteNome: _comandaNameController.text,
                  itens: [],
                  status: 'open',
                  pagamentos: [],
                  date: todayStr,
                ...
                ref.read(comandaControllerProvider.notifier).addComanda(newComanda);
                Navigator.pop(ctx);
              }
            },
          ),
  ...
  void _showAddItemDialog(BuildContext context, Comanda comanda) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            backgroundColor: ThemeColors.darkBg,
            title: Text('Lançar na comanda de ${comanda.clienteNome}', style: const TextStyle(color: Colors.white, fontSize: 16)),
            content: SizedBox(
              width: 400,
              child: Column(
              ...
              AppButton(
                ...
                onPressed: () {
                  if (selectedItemId != null) {
                    ...
                    ref.read(comandaControllerProvider.notifier).updateComanda(updated);
                    Navigator.pop(ctx);
                  }
                },
              ),
  ...
  void _showCheckoutDialog(BuildContext context, Comanda comanda) {
    double discount = 0.0;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) {
          final subtotal = comanda.subtotal;
          ...
          return AlertDialog(
            backgroundColor: ThemeColors.darkBg,
            title: Text('Fechar Comanda: ${comanda.clienteNome}', style: const TextStyle(color: Colors.white, fontSize: 16)),
            content: SizedBox(
              width: 450,
            ...
            actions: [
              ...
              AppButton(
                ...
                onPressed: () {
                  updateValues();
                  final List<PagamentoSplit> payments = [];
                  if (pixAmount > 0) payments.add(PagamentoSplit(method: 'pix', amount: pixAmount));
                  if (cardAmount > 0) payments.add(PagamentoSplit(method: 'credit', amount: cardAmount));
                  if (cashAmount > 0) payments.add(PagamentoSplit(method: 'money', amount: cashAmount));

                  // Fallback to cash if none specified
                  if (payments.isEmpty) {
                    payments.add(PagamentoSplit(method: 'money', amount: total));
                  }

                  ref.read(comandaControllerProvider.notifier).checkoutComanda(comanda, payments, discount);
                  Navigator.pop(ctx);
                },
              ),
            ],
